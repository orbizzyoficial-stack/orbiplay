...
export async function onRequest(context){
  ...
  const user = await env.DB.prepare(
    "SELECT email,name,pass_hash,salt FROM users WHERE email=?"
  ).bind(email).first();

  if (!user){
    return json(request, { ok:false, error:"Email ou senha inválidos" }, 401);
  }
  ...
  const token = await signToken({ email, ts: Date.now() }, secret);

  return json(request, { ok:true, token, profile: { email, name: user.name || "" } });
}
